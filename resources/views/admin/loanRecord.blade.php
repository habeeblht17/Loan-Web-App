<x-admin-layout>
    ...
    <!--Loan Search -->
    @if ($results)
    <div class="p-6 overflow-auto bg-white rounded-md shadow dark:bg-dark-eval-1 dark:text-gray-400">
            ...
            <div class="p-4 bg-violet-50 dark:bg-dark-eval-1">
                <div class=" w-full">
                    <div class="flex flex-col justify-between md:flex-row">
                        <div class="w-1/2">
                            <h1 class="p-1 text-xl text-gray-500 dark:text-gray-100 font-semibold">Loan Result</h1>
                        </div>
                    </div>
                    <!-- PaidLoan -->
                    <table class="w-full rounded-lg bg-violet-50 dark:bg-dark-bg">
                        ...
                        <tbody class="w-full divide-y divide-gray-300">
                            ...
                            <tr class="">
                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->user->id }}</td>

                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->user->firstname }}</td>

                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->user->lastname }}</td>

                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->amount }}</td>

                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->description }}</td>

                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->status }}</td>

                                <td class="p-3 text-sm text-center font-semibold  text-gray-400">{{ $result->created_at }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
